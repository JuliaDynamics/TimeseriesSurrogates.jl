...
export PseudoPeriodicTwin2

struct PseudoPeriodicTwin2{T<:Real, P<:Real, D<:PreMetric} <: Surrogate
    d::Union{Nothing, Int}
    τ::Union{Nothing, Int}
    δ::T
    ρ::P
    ...
    function PseudoPeriodicTwin2(d::Int, τ::Int, δ::T = 0.2, ρ::P = 0.1, metric::D = Euclidean()) where {T, P, D}
        new{T, P, D}(d, τ, δ, ρ, metric)
    end
    
    function PseudoPeriodicTwin2(δ::T = 0.2, ρ::P = 0.1, metric::D = Euclidean()) where {T, P, D}
        new{T, P, D}(nothing, nothing, δ, ρ, metric)
    end
end
...
function surrogenerator(x::Union{AbstractVector, Dataset}, pp::PseudoPeriodicTwin2, rng = Random.default_rng())
    d, τ, δ, metric = getfield.(Ref(pp), (:d, :τ, :δ, :metric))
    ρ = getfield.(Ref(pp), (:ρ))
...
function (sg::SurrogateGenerator2{<:PseudoPeriodicTwin2})()
    pts, Nx, Npts, twins, W = getfield.(Ref(sg.init), (:pts, :Nx, :Npts, :twins, :W))
    ρ = getfield.(Ref(sg.method), (:ρ))
    s = sg.s
