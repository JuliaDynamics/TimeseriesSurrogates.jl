...
2. Local maxima of the smoothened signal define the peaks, and thus the blocks in between them.
  (notice that first and last index of timeseries can never be peaks and thus signals that
  should have peaks very close to start or end may not perform well)
...
surrogenerator(x::AbstractVector, bs::CycleShuffle) = SurrogateGenerator(bs, x, nothing)

function (sg::SurrogateGenerator{<:CycleShuffle})()
    x, n, N = sg.x, sg.method.n, length(sg.x)
    r = iseven(n) ? (n÷2:N-n÷2) : (n÷2:N-n÷2-1)
    g = DSP.gaussian(n, sg.method.σ)[n÷2:end-n÷2]
    smooth = DSP.conv(x, g)
    r = length(smooth) - N
    smooth = iseven(r) ? smooth[r÷2+1:end-r÷2] : smooth[r÷2+1:end-r÷2-1]
    peaks = findall(i -> smooth[i-1] < smooth[i] && smooth[i] > smooth[i+1], 1:N)
    # now do the block shuffle.
    return s
end
