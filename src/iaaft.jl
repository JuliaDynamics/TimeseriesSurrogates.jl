...
function iaaft(ts; n_maxiter = 200, tol = 1e-6, n_windows = 50)
    ...
    # RANK ORDERING.
    # Create some Gaussian noise, and find the indices that sorts it with increasing
    # amplitude. Then sort the original time series according to the indices rendering
    # the Gaussian noise sorted.
    n = length(ts)
    g = rand(Normal(), n)
    inds = sortperm(g)
    ts_sorted = ts[inds]
    ...
    spectrum = Vector{Complex64}(n)
    surrogate = Vector{Float64}(n)

    diffs = zeros(Float64, 2)
    while iter <= n_maxiter
        # Take the Fourier transform of `ts_sorted` and get the phase angles of the
        # resulting complex numbers.
        FT = fft(ts_sorted)
        phase_angles = angle.(FT)

        # The new spectrum preserves the amplitudes of the Fourier transform of the
        # original time series, but randomises the phases (because the phases are derived
        # from the *randomly sorted* version of the original time series).
        spectrum[:] = original_fft_amplitudes .* exp.(phase_angles .* 1im)

        # Now, let the surrogate time series be the values of the original time series,
        # but sorted according to the new spectrum. The shuffled series is generated by
        # taking the inverse Fourier transform of the spectrum consisting of the original
        # amplitudes, but having randomised phases.
        ...
        power_binned = intp([x for x in periodogram.freq], periodogram.power, n_windows)
        power_binned_surr = intp([x for x in periodogram_surr.freq], periodogram_surr.power, n_windows)

        if iter == 1
            diffs[1] = sum((power_binned[2] .- power_binned_surr[2]).^2) / sum(power_binned[2].^2)
        else
            diffs[2] = sum((power_binned[2] .- power_binned_surr[2]).^2) / sum(power_binned[2].^2)
...
function iaaft_iters(ts; n_maxiter = 100, tol = 1e-5, n_windows = 50)
    ...
    spectrum = Vector{Complex64}(n)
    surrogate = Vector{Float64}(n)
    surrogates = Vector{Vector{Float64}}(0)
