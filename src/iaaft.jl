...
IAAFT(;n_maxiter::Int = 100, tol::Real = 1e-6, n_windows::Int = 75) = 
    IAAFT(nothing, nothing, n_maxiter, tol, n_windows)
...
function surrogate(s, method::IAAFT) 
    isnothing(method.forward) ? forward = plan_rfft(s) : forward = method.forward
    isnothing(method.inverse) ? inverse = plan_irfft(forward*s, length(s)) : inverse = method.inverse
    ...
    while iter <= method.n_maxiter
        ...
        surr .= real.(inverse * 𝓕new) # ifft normalises by default
