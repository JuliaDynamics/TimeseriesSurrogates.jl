...
for surrogate_type in surrogate_methods
    ...
    @eval begin
        ...
        """
        function $(fname2b)(ts; filename = nothing, fps = 2, n_iters = 15)
            if $(surrtype) == :PseudoPeriodic
                d, τ, ρ = 3, 10, 0.1
                method = PseudoPeriodic(d, τ, ρ)
            end
    ...
    for process in processes
        ...
        @eval begin
            """
                ...
                If `new_realization_every_iter = true`, then a fresh `$($proc)` realization is used at each iteration. Otherwise, the same time series, generating only the surrogates anew at each iteration.
            """
            function $(fname4b)(;filename = nothing, fps = 2, n_iters = 15,
                                n_steps = 500,
                                new_realization_every_iter::Bool = false)
                # generate time series
                ts = $(proc)(n_steps = n_steps)
    ...
    for process in processes
        ...
        @eval begin
            """
                ...
                If `new_realization_every_iter = true`, then a fresh `$($proc)` realization is used at each iteration. Otherwise, the same time series, generating only the surrogates anew at each iteration.
            """
            function $(fname_gif)(; filename = nothing, fps = 2, n_iters = 15,
                                n_steps = 500,
                ...
                if isnothing(filename)
                    Plots.gif(anim, fps = fps; kwargs...)
                else
                    Plots.gif(anim, filename, fps = fps; kwargs...) 
                end
            end
        end
        ...
        @eval begin
            """
                ...
                If `new_realization_every_iter = true`, then a fresh `$($proc)` realization is used at each iteration. Otherwise, the same time series, generating only the surrogates anew at each iteration.
            """
            function $(fname_mp4)(;filename, fps = 2, n_iters = 15,
                                n_steps = 500,
                                new_realization_every_iter::Bool = false, kwargs...)
                # generate time series
                ts = $(proc)(n_steps = n_steps)
                ...
                if isnothing(filename)
                    Plots.mp4(anim, fps = fps; kwargs...)
                else
                    Plots.mp4(anim, filename, fps = fps; kwargs...) 
                end
            end
        end
